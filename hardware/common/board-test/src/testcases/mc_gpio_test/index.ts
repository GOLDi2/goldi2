...
const testCaseData: TestCaseData = {
    ...
    test: (tc: TestCase) => {
        return new Promise<void>(resolve => {
            const proc_0 = spawnSync(`svf-player ${generatedTestsDir}/test_fpga_mc_0.svf -ni`, {timeout: 10000, shell: true})
            if (proc_0.status != 0) {
                tc.outcome = "Fail"
            ...
            const proc_1 = spawnSync(`svf-player ${generatedTestsDir}/test_fpga_mc_1.svf -ni`, {timeout: 10000, shell: true})
            if (proc_1.status != 0) {
                tc.outcome = "Fail"
                const str = fs.readFileSync("/tmp/svf_output.json", {encoding: "utf-8"})
                const faults = JSON.parse(str)["faults"]
