...
export function createProcessInterface(router: Router, base: string, createProcess?: (() => ChildProcessWithoutNullStreams)) {
    ...
    function prepareProcess(process: ChildProcessWithoutNullStreams) {
        ...
        pending_process?.process.on("close", () => { pending_process = undefined });
        pending_process?.process.stdin.on("error", () => {});
    }

    router.get(base + '/stream', (req, res) => {
        res.writeHead(200, {
        ...
        if (!pending_process) {
            if (!createProcess) { res.end(); return; }
        ...
        pending_process?.process.stdout.on("data", (data) => { res.write('data: ' + JSON.stringify(data.toString()) + "\n\n"); });
        pending_process?.process.stderr.on("data", (data) => { res.write('data: ' + JSON.stringify(data.toString()) + "\n\n"); });
        pending_process?.process.on('close', function (code) { res.end(); });
    });

    router.get(base + '/input', (req, res) => {
        if (!pending_process) {
            if (!createProcess) { res.sendStatus(404); return; }
            prepareProcess(createProcess())
        }
        try{
    ...
    return {
        setProcess: prepareProcess, isRunning: () => pending_process !== undefined
    }
}