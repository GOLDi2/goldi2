        ...
        if (rst = '1') then
            ...
            case ps_i2c is
                when s_idle =>          --Idle state holds for a "start" tagged data word to be transfered
                    if (p_tdword_tvalid = '1' and p_tdword_start = '1') then
                        ps_i2c <= s_start;
                    else
                        ps_i2c <= s_idle;
                    end if;
                ...
                when s_start =>         --Generate a starting condition by driving sda low while scl is high
                    if (i2c_clk_counter = g_scl_factor - 1) then
                        ps_i2c <= s_address;
                    else
                        ps_i2c <= s_start;
                    end if;
                ...
                when s_address =>       --Transmit the address word to select slave
                    if (i2c_clk_counter = g_scl_factor - 1 and i2c_t_valid = '1') then
                        ps_i2c <= s_hold;
                    elsif (i2c_clk_counter = g_scl_factor - 1 and i2c_t_error = '1') then
                        ps_i2c <= s_idle;
                    else
                        ps_i2c <= s_address;
                    end if;
                ...
                when s_hold =>          --Wait for next data word to process
                    if (p_tdword_tvalid = '1' and p_tdword_start = '1') then
                        ps_i2c <= s_start_setup;
                    elsif (p_tdword_tvalid = '1' and p_tdword_stop = '1' and tdword_wr_buffer = '1') then
                        ps_i2c <= s_rstop;
                    elsif (p_tdword_tvalid = '1' and p_tdword_stop = '1' and tdword_wr_buffer = '0') then
                        ps_i2c <= s_tstop;
                    elsif (p_tdword_tvalid = '1' and tdword_wr_buffer = '1') then
                        ps_i2c <= s_rhold;
                    elsif (p_tdword_tvalid = '1' and tdword_wr_buffer = '0') then
                        ps_i2c <= s_thold;
                    else
                        ps_i2c <= s_hold;
                    end if;
                ...
                when s_rhold =>         --Recive data and return to hold state
                    if (i2c_clk_counter = g_scl_factor - 1 and i2c_r_valid = '1') then
                        ps_i2c <= s_hold;
                    else
                        ps_i2c <= s_rhold;
                    end if;
