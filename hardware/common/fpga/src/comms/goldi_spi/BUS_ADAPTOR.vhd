...
begin
    ...
    begin
        if (rst = '1') then
            ...
            case ps_mbus is
                when s_idle => if (p_nce /= '0') then
                        ps_mbus <= s_idle;
                    else
                        ps_mbus <= s_config;
                    end if;

                when s_config => if (p_nce /= '0') then
                        ps_mbus <= s_idle;
                    elsif (p_word_val = '1') then
                        ps_mbus <= s_data;
                    else
                        ps_mbus <= s_config;
                    end if;

                when s_data => if (p_nce /= '0') then
                        ps_mbus <= s_idle;
                    elsif (p_word_val = '1' and we_buffer = '1') then
                        ps_mbus <= s_wvalid;
                    elsif (p_word_val = '1' and we_buffer = '0') then
                        ps_mbus <= s_rvalid;
                    else
                        ps_mbus <= s_data;
                    end if;

                when s_rvalid => if (p_nce /= '0') then
                        ps_mbus <= s_idle;
                    else
                        ps_mbus <= s_data;
                    end if;

                when s_wvalid => if (p_nce /= '0') then
                        ps_mbus <= s_idle;
                    else
                        ps_mbus <= s_data;
                    end if;
