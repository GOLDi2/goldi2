...
architecture RTL of UART_TX_DDRIVER is
    ...
    --Counter
    signal tx_clk_period  : integer range 0 to g_clk_frequency / 9600;
    signal tx_clk_counter : integer range 0 to g_clk_frequency / 9600;
...
begin
        ...
        if (rst = '1') then
            ...
            case ps_tx is
                ...
                when s_transfer =>      --Transfer state shifts the codified serial data
                    if (tx_clk_counter = 0 and tx_cyc_counter = g_packet_width - 1) then
                        ps_tx <= s_buffer;
                    else
                        ps_tx <= s_transfer;
                    end if;
                ...
                when s_buffer =>        --Enable ready flag after last data bit to allow data queuing
                    if (tx_clk_counter = tx_clk_period - 1 and tx_cyc_counter = g_packet_width - 1) then
                        ps_tx <= s_idle;
                    elsif (p_dword_tvalid = '1') then
                        ps_tx <= s_hold;
                    else
                        ps_tx <= s_buffer;
                    end if;
    ...
    -------------------------------------------------------------------------------------------------------------------
    with drate_buffer_i select tx_clk_period <=
        baud_table(0) when "000",
    ...
    begin
        if (rst = '1') then
            ...
            if (ps_tx = s_idle) then
                p_tx <= '1';
            elsif ((tx_clk_counter = 0) and (g_msbf)) then
                p_tx <= dword_buffer_i((g_packet_width - 1) - tx_cyc_counter);
            elsif ((tx_clk_counter = 0) and (not g_msbf)) then
