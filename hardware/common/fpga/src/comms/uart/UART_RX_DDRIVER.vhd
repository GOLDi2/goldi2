...
architecture RTL of UART_RX_DDRIVER is
    ...
    --Timing counters
    signal rx_clk_period  : integer range 0 to g_clk_frequency / 9600;
    signal rx_clk_counter : integer range 0 to g_clk_frequency / 9600;
...
begin
        ...
        if (rst = '1') then
            ...
            case ps_rx is
                ...
                when s_detect =>        --Sample uart port a second time to ensure valid transaction
                    if (rx_clk_counter = rx_clk_period / 4 and p_rx = '0') then
                        ps_rx <= s_sample;
                    elsif (rx_clk_counter = rx_clk_period / 4) then
                        ps_rx <= s_idle;
                    else
                        ps_rx <= s_detect;
                    end if;
                ...
                when s_sample =>        --Sample data at the middle of the uart period
                    if (rx_clk_counter = rx_clk_period - 1 and rx_cyc_counter = g_packet_width - 1 and p_rx = '0') then
                        ps_rx <= s_detect;
    ...
    -------------------------------------------------------------------------------------------------------------------
    with drate_buffer_i select rx_clk_period <=
    ...
    begin
        if (rst = '1') then
            ...
            if (rx_clk_counter = rx_clk_period / 2 and g_msbf) then
