...
architecture BH of REGISTER_TABLE is
	...
	--****INTERNAL SIGNALS****
	--Address constants
	constant min_address : signed(BUS_ADDRESS_WIDTH downto 0) := to_signed(g_address, BUS_ADDRESS_WIDTH + 1);
	constant max_address : signed(BUS_ADDRESS_WIDTH downto 0) := to_signed(g_address + g_reg_number, BUS_ADDRESS_WIDTH + 1);
...
begin
	...
	begin
		if (rst = '1') then
			...
			if ((min_address <= signed(bus_adr)) and (signed(bus_adr) < max_address) and (sys_bus_i.we = '0') and (sys_bus_i.stb = '1')) then
			...
			elsif ((min_address <= signed(bus_adr)) and (signed(bus_adr) < max_address) and (sys_bus_i.we = '0')) then
	...
	begin
		if (rst = '1') then
			...
			if ((min_address <= signed(bus_adr)) and (signed(bus_adr) < max_address) and (sys_bus_i.we = '1') and (sys_bus_i.stb = '1')) then
