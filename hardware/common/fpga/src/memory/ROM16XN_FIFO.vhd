...
architecture RTL of ROM16XN_FIFO is
    ...
    --****COMPONENTS****
    component ROM16X1A                  -- @suppress "Component declaration 'ROM16X1A' has none or multiple matching entity declarations"
        generic(
...
begin
    ...
    begin
        if (rst = '1') then
            ...
            case ps_fifo is
                when s_delay =>
                    if (delay_counter = g_init_delay - 1) then
                        ps_fifo <= s_load;
                    else
                        ps_fifo <= s_delay;
                    end if;
                ...
                when s_idle =>          -- @suppress "Dead state 's_idle': state does not have outgoing transitions"
                    ps_fifo <= s_idle;
            end case;
        end if;
    end process;
