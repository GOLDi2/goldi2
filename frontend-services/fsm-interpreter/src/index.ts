...
type Equation = {raw: string, error?: string, parsed?: {variable: string, fun: (input: Object)=>boolean}}
...
export class App extends LitElement {
  @state()
  private equations: {z: Equation[], y: Equation[]} = {z: [{raw: ""}], y: [{raw: ""}]};
  @state()
  ...
  private changeCallbacks: Map<string, ((value: boolean)=>void)[]> = new Map();
  ...
  changeEquation(key: 'z'|'y', idx: number, value: string){
    ...
    this.equations={...this.equations, [key]: [...this.equations[key].filter((equation)=>equation.raw!==""), {raw: ""}]};
  }

  step(){
    let output: Record<string, boolean> = {};
    for (const equation of [...this.equations.z, ...this.equations.y].filter(e=>e.parsed)){
      try{
  ...
  renderEquations(key: 'z' | 'y', name: string){
    ...
    ${name} <br><br>
    ${this.equations[key].map((equation, idx)=>html`
      ${equation.error ? html`<div class="text-red-500 pb-2">${equation.error}</div>` : html``}
      <input value="${equation.raw}" class="p-2 rounded text-black w-full" type="text" @input="${(event: any)=>this.changeEquation(key, idx, event.target.value)}" > <br><br>
    `)}
  </div>
    `
  }
  ...
  render() {
    return html`
      <div class="h-full flex flex-col" @webcam="${() => {this.isLoading = false;}}">
        ...
        <div class="grow flex w-full items-center flex-col p-2 max-w-4xl m-auto">
          <div class="bg-primary-100 p-4 m-2 rounded-md text-black">
            variables: <br><br>
            <div class="flex flex-wrap">
            ${Object.entries(this.input).map(([variable, value])=>html`
              <div class="w-32"> ${variable}: ${value ? "true" : "false"} </div>
            `)}
            </div>
          </div>
        ...
        <div class="bg-primary-100 w-full h-12 flex flex-row align-middle justify-center">
          <button class="bg-primary m-2 p-2 rounded hover:bg-primary-100" @click="${()=>this.reset()}">Reset</button>
          <button class="bg-primary m-2 p-2 rounded hover:bg-primary-100" @click="${()=>this.step()}">Step</button>
          <button class="${this.interval?'bg-secondary':'bg-primary'} m-2 p-2 rounded hover:bg-primary-100" @click="${()=>this.toggle_run()}">Run</button>
        </div>
      </div>
      ${
        this.isLoading
