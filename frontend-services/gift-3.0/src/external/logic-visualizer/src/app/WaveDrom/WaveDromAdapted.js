...
export function renderWaveForm(source, output, domain, fullRedraw) {
    let index = 0; // LV only needs one SVG
    var ret, root, groups, svgcontent, content, width, height, glengths, xmax = 0, i;
    if (fullRedraw) {
        prevLaneElements = [];
        prevRenderedBricks = [];
    }
...
function renderWaveLane(root, content, index, hStar, numberOfInputs, fullRedraw) {
    var i, j, // index iterates over content
    k, g, gg, title, brick, labels = [1], name, xmax = 0, xgmax = 0, glengths = [], svgns = 'http://www.w3.org/2000/svg', xlinkns = 'http://www.w3.org/1999/xlink', xmlns = 'http://www.w3.org/XML/1998/namespace';
...
function renderArcs(root, source, index, top) {
    var gg, i, k, text, Stack = [], Edge = { words: [], from: 0, shape: '', to: 0, label: '' }, Events = {}, pos, eventname, 
    // labeltext,
    label, underlabel, from, to, gmark, lwidth, svgns = 'http://www.w3.org/2000/svg', xmlns = 'http://www.w3.org/XML/1998/namespace';
        ...
        if (top.edge) {
            for (i in top.edge) {
                ...
                switch (Edge.shape) {
                    ...
                    default: {
                        gmark.setAttribute('style', 'fill:none;stroke:#F00;stroke-width:1');
                    }
                }
                if (Edge.label) {
...
function renderGaps(root, source, index) {
    var i, gg, g, b, pos, Stack = [], text, svgns = 'http://www.w3.org/2000/svg', xlinkns = 'http://www.w3.org/1999/xlink';
    if (source) {
        gg = document.createElementNS(svgns, 'g');
...
function renderGroups(root, groups, index) {
    ...
    svgns = 'http://www.w3.org/2000/svg', xmlns = 'http://www.w3.org/XML/1998/namespace';
    for (i in groups) {
...
function renderAssign(index, source, domain) {
    ...
    function draw_body(type, ymin, ymax) {
        var e, l, iecs, circle = ' M 4,0 C 4,1.1 3.1,2 2,2 0.9,2 0,1.1 0,0 c 0,-1.1 0.9,-2 2,-2 1.1,0 2,0.9 2,2 z', gates = {
    ...
    function draw_gate(spec) {
        var i, ret = [], ys = [], ymin, ymax, ilen = spec.length;
        ret.push('g');
        for (i = 2; i < ilen; i++) {
            ys.push(spec[i][1]);
        }
        ...
        ret.push(['g', { transform: 'translate(' + spec[1][0] + ',' + spec[1][1] + ')' },
            ['title', spec[0]],
    ...
    function draw_boxes(tree, xmax) {
        ...
        if (Object.prototype.toString.call(tree) === '[object Array]') {
            ...
            fx = 32 * (xmax - tree.x);
            fy = 8 * tree.y;
            ret.push(['g', { transform: 'translate(' + fx + ',' + fy + ')' },
                ['title', fname],
...
function genFirstWaveBrick(instructionChar, extra, times) {
    ...
    switch (instructionChar) {
        case 'p':
            returnValue = genBrick(['pclk', '111', 'nclk', '000'], extra, times);
            break;
        case 'n':
            returnValue = genBrick(['nclk', '000', 'pclk', '111'], extra, times);
            break;
        case 'P':
            returnValue = genBrick(['Pclk', '111', 'nclk', '000'], extra, times);
            break;
        ...
        case 'L':
        case '0':
            returnValue = genBrick(['000'], extra, times);
            break;
        case 'h':
        case 'H':
        case '1':
            returnValue = genBrick(['111'], extra, times);
            break;
        case '=':
            returnValue = genBrick(['vvv-2'], extra, times);
            break;
        case '2':
            returnValue = genBrick(['vvv-2'], extra, times);
            break;
        case '3':
            returnValue = genBrick(['vvv-3'], extra, times);
            break;
        case '4':
            returnValue = genBrick(['vvv-4'], extra, times);
            break;
        case '5':
            returnValue = genBrick(['vvv-5'], extra, times);
            break;
        case 'd':
            returnValue = genBrick(['ddd'], extra, times);
            break;
        case 'u':
            returnValue = genBrick(['uuu'], extra, times);
            break;
        case 'z':
            returnValue = genBrick(['zzz'], extra, times);
            break;
        default:
            returnValue = genBrick(['xxx'], extra, times);
            break;
    }
    return returnValue;
}
...
//# sourceMappingURL=WaveDromAdapted.js.map