...
import { dispatchInitializeVisualizerEvent, dispatchSimulatorChangeEvent } from "../app/model/Events";
class FSM {
    constructor(stateVariableEquationStringList, environment) {
        //TODO LATER switch to arrow functions
        //TODO LATER typings
        this.stateVariables = []; // current value of z4 is stateVariables[4].value
        //TODO empty state variables list => constant z0
        stateVariableEquationStringList.forEach((equationString) => {
    ...
    getStateNumber() {
        let rv = 0;
        this.stateVariables.forEach((sVar, index) => {
            if (sVar.value) {
                rv += 2 ** index;
            }
        });
...
export class FSMEnvironment {
    constructor(inputVariableCount, outputVariableEquationStringList, machineStateEquationStringList) {
        this.fsmList = [];
        this.inputVariables = [];
        this.outputVariables = []; // current value of y5 is outputVariables[5].value
        ...
        outputVariableEquationStringList.forEach((equationString) => {
            let equation = stringToLogicExpression(equationString, this);
            this.outputVariables.push(new Variable(false, equation));
        });
        machineStateEquationStringList.forEach((statesEquationStringList) => {
        ...
        document.addEventListener('lv-user-change', (e) => {
            ...
            let args = { outputs: [], answeringClock: false, hStar: "0" };
            if (e.args.inputs !== undefined) {
                e.args.inputs.forEach((input) => {
            ...
            if (e.args.inputs !== undefined || e.args.states !== undefined) {
                ...
                this.outputVariables.forEach((oVar, index) => {
        ...
        document.addEventListener('lv-clock', (e) => {
            ...
            this.fsmList.forEach((fsm, index) => {
        ...
        document.addEventListener('lv-reset', (e) => {
            ...
            this.initOutputVariableEquationStringList.forEach((equationString) => {
                let equation = stringToLogicExpression(equationString, this);
                this.outputVariables.push(new Variable(false, equation));
            });
            this.fsmList = [];
            this.initMachineStateEquationStringList.forEach((statesEquationStringList) => {
                this.addFSM(statesEquationStringList);
            });
            this.calculateOutput();
