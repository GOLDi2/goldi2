...
const MONACO_EDITOR_VERSION = (function() {
	const packageJsonPath = path.join(__dirname, 'node_modules/monaco-editor/package.json');
...
const cleanReleaseTask = function(cb) { rimraf('editor-release', { maxBusyTries: 1 }, cb); };
gulp.task('release', taskSeries(cleanReleaseTask, function() {
	return es.merge(
		...
		gulp.src('package.json')
			...
			.pipe(gulp.dest('editor-release')),

		gulp.src('node_modules/monaco-editor/CHANGELOG.md')
			.pipe(gulp.dest('editor-release')),

		// min-maps folder
		gulp.src('node_modules/monaco-editor-core/min-maps/**/*')
			.pipe(gulp.dest('editor-release/min-maps')),
		...
		gulp.src([
			...
			'node_modules/monaco-editor/README.md'
		])
		.pipe(addPluginDTS())
...
function releaseOne(type) {
	return es.merge(
		gulp.src('node_modules/monaco-editor-core/' + type + '/**/*')
			...
			.pipe(gulp.dest('editor-release/' + type)),

		pluginStreams(type, 'editor-release/' + type + '/')
	)
}
...
function ESM_release() {
	return es.merge(
		gulp.src([
			...
			.pipe(ESM_addPluginContribs('editor-release/esm'))
			.pipe(gulp.dest('editor-release/esm')),

		ESM_pluginStreams('editor-release/esm/')
	)
}
...
function addPluginDTS() {
	return es.through(function(data) {
		...
		fs.writeFileSync('node_modules/monaco-editor/monaco.d.ts', contents);
		this.emit('data', data);
	});
}
	...
	return (
		es.merge(
			...
			.pipe(es.through(function(data) {
				...
				var contents = data.contents.toString();
				contents = contents.replace(/\.\.\/editor-release\/dev/g, 'node_modules/monaco-editor/min');
