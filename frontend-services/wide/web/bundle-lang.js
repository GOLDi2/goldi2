...
const REPO_ROOT = path.resolve(__dirname, 'monaco-languages');

const sha1 = helpers.getGitVersion(REPO_ROOT);
const semver = require('monaco-languages/package.json').version;
...
bundleOne('vhdl/vhdl');
bundleOne('logic/logic');

function bundleOne(moduleId, exclude) {
	requirejs.optimize({
		baseUrl: 'monaco-languages/release/dev/',
		name: 'vs/basic-languages/' + moduleId,
		out: 'monaco-languages/release/min/' + moduleId + '.js',
		exclude: exclude,
		paths: {
