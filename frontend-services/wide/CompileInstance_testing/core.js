const Queue = require('./res/queue.js.js');
const Task = require('./res/task.js.js');
const fs = require('fs');
const http = require('http');
const responseHandler = require('./res/responseHandler');
...
function compileRequestHandler (request, resStream) {
    let body = [];
    request.on('data', (chunk) => {
        ...
        if(!compiler.hasOwnProperty(language)){
            resStream.end('language not supported');
            return;
        }
        let JSONbody;
        try {
        ...
        let resHandler = new responseHandler(resStream);
        let task = new Task(0, language, JSONbody, resHandler, false);
...
async function compile(task) {
    await compiler[task.language](task);
    // TODO get this into the task as function that gets called when the compilation has finished
    clearInterval(task.sendEstimateInterval);
