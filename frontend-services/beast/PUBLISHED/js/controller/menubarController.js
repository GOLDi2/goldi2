...
class MenubarController {
    ...
    constructor(beastController) {
        ...
        $('#createComponentFromSequence')
            .on('click', { sequenceController: this.createSequenceController, beastController: this.beastController }, this.createComponentFromSequence);
    ...
    createComponentFromEquation(event) {
        const controller = event.data.controller;
        InputDialog.type = 'createComponent';
        const dialog = new EquationComponentCreationDialog((input, expression) => {
            try {
    ...
    createComponentFromSequence(event) {
        const sequenceController = event.data.sequenceController;
        const beastController = event.data.beastController;
...
class EquationComponentCreationDialog extends Dialog {
    constructor(callbackFunction, defaultExpression) {
        super({
            width: '700px'
        });
