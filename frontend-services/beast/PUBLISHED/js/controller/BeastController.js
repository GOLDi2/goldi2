...
class BeastController {
     ...
     * @return {}
     */
    createComponentAndPutInLibrary(component, name) {
        let componentGlobalID = this.createComponent(component);
        if (!componentGlobalID) {
            return null;
        }
        let library = this.createOrGetLibraryByName(name);
    ...
    deleteSingleComponent(key) {
        ...
        let comp = currentProject.circuit;
        if (comp.directlyDependsOn(key)) {
            blockingDependencies.push({
                componentID: comp.ID,
            });
