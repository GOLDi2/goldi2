...
!function ($, $s) {
    ...
    var createSwitchFactory = function (type) {
        return function (device) {
            ...
            device.createUI = function () {
                ...
                var $button = $s.createSVGElement('rect').attr({
    ...
    var createRotaryEncoderFactory = function () {
        ...
        return function (device) {
            ...
            device.createUI = function () {
                ...
                var $knob = $s.createSVGElement('g').attr('class', 'simcir-basicset-knob').append($s.createSVGElement('rect').attr({
                    x: -10,
                    y: -10,
                    width: 20,
                    height: 20
    ...
    $s.registerDevice('LED', function (device) {
        ...
        device.createUI = function () {
            ...
            setParameters();
            var size = device.getSize();
            var $ledbase = $s.createSVGElement('circle').attr({
                cx: size.width / 2,
                cy: size.height / 2,
                r: size.width / 4
            }).attr('stroke', 'none').attr('fill', bLoColor);
            device.$ui.append($ledbase);
            var $led = $s.createSVGElement('circle').attr({
                cx: size.width / 2,
                cy: size.height / 2,
                r: size.width / 4 * 0.8
    ...
    $s.registerDevice('Label', function (device) {
