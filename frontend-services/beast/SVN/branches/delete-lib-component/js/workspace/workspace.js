...
var simcir = function ($) {
    ...
    var rotateCoords = function (p, angle) {
        ...
        return { x: Math.round(Math.cos(radAngle) * p.x - Math.sin(radAngle) * p.y),
    ...
    var offset = function ($o, prex, prey) {
        var x = prex || 0;
        var y = prey || 0;
        while ($o[0].nodeName != 'svg' && !$o.hasClass("simcir-containerPane")) {
            var pos = transform($o);
    ...
    var createDeviceController = function (device) {
        ...
        var createUI = function () {
            ...
            device.$ui.on('deviceAdd', () => {
                if (device.doc == null || device.doc.haslabel === undefined || device.doc.haslabel == true)
                    device.$ui.closest(".simcir-devicepane").append($label);
        ...
        var openParameterDialog = function () {
            ...
            if (device.doc == null || device.doc.haslabel === undefined || device.doc.haslabel == true) {
                let labledef = {
                    ...
                    displayName: "Label",
                    defaultValue: device.deviceDef.type
                };
                params = [labledef].concat(params);
            }
            let paramDialog = new ParameterDialog(params, device.deviceDef, setValues);
            paramDialog.show();
        };
        var $tooltip = null;
        ...
        var getAnchorBelow = function () {
            //TODO: Correct for rotated non-sqare devices
            const bb = device.$ui[0].getBBox();
        ...
        var updateTooltipPosition = function () {
            if ($tooltip !== null) {
                ...
                device.$ui.closest(".simcir-containerPane").append($tooltip);
            }
        };
    ...
    var createWorkspace = function (data, $container) {
        data = $.extend({
            width: 400,
            height: 200,
            showToolbox: true,
        ...
        var $workspace = createSVG().attr('class', 'simcir-workspace');
        disableSelection($workspace);
        var $defs = createSVGElement('defs');
        var $containerPane = createSVGElement('g').addClass("simcir-containerPane");
        ...
        !function () {
            ...
            $defs.append(createSVGElement('pattern').
                ...
                width: patternPitch / w, height: patternPitch / h }).append(createSVGElement('rect').attr('class', 'simcir-pin-hole').
        ...
        controller($scrollbar).setSize(barWidth, 500);
        ...
        var addDevice = function ($dev) {
            ...
            controller($dev).setShowConnectorState(showConnectorState);
            for (let $node of $dev.find(".simcir-node").toArray())
                controller($($node)).setShowConnectorState(showConnectorState);
        };
        ...
        var loadToolbox = function (data) {
            ...
            controller($scrollbar).setValues(0, 0, y, 500);
        };
        var devicesCenterOfGravity = function ($devices) {
            let xsum = 0, ysum = 0;
            for (let $dev of $devices) {
                const size = controller($($dev)).getSize();
        ...
        var tranformCoords = function (p) {
            ...
            return { x: (p.x - globalTransform.x) / globalTransform.zoom, y: (p.y - globalTransform.y) / globalTransform.zoom };
        };
        ...
        var beginConnect = function (event, $target) {
            ...
            var insertJunction = function (event) {
                ...
                transform($joint, x - toolboxWidth - portoffet.x, y - portoffet.y, theta);
                adjustDevice($joint);
                updateConnectors();
        ...
        var beginNewDevice = function (event, $target) {
            ...
            transform($dev, pos.x, pos.y, transform($dev).rotate);
            ...
            dragMoveHandler = function (event) {
                transform($dev, event.pageX - dragPoint.x, event.pageY - dragPoint.y, transform($dev).rotate);
                controller($dev).updatePosition();
            };
            dragCompleteHandler = function (event) {
                ...
                if ($target.closest('.simcir-toolbox').length == 0) {
                    $dev.detach();
                    var pos = transform($dev);
                    transform($dev, pos.x - toolboxWidth, pos.y, transform($dev).rotate);
                    adjustDevice($dev);
                    addDevice($dev);
                    markDirty($dev);
                    controller($dev).updatePosition();
        ...
        var setShowConnectorState = function (showState) {
            ...
            updateConnectors();
            for (let $dev of $devicePane.find(".simcir-device").toArray()) {
                controller($($dev)).setShowConnectorState(showState);
            }
            for (let $node of $devicePane.find(".simcir-node").toArray()) {
                controller($($node)).setShowConnectorState(showState);
            }
        };
        var zoomRelative = function (zoomfactor, center) {
            center = tranformCoords(center || { x: $workspace.width() / 2, y: $workspace.height() / 2 });
            const pos = transform($containerPane);
            const scalechange = (pos.zoom * zoomfactor) - pos.zoom;
            pos.zoom *= zoomfactor;
        ...
        $workspace.on('wheel', (event) => zoomRelative(1 + Math.sign(event.originalEvent.deltaY) / 10, getEventPos(event)));
        var $draggedDevice = null;
        $container.droppable({
            /*activate: function(event, ui) {
                //ui.dragable.refreshPositions = true;
                //TODO: Use real device
                let dev = {type: "LED"};
                $draggedDevice = createDevice(dev, false, scope);
            ...
            over: function(event, ui){
                 ...
                 //controller($draggedDevice).updatePosition();
            },
            ...
            /*activate: function(event, ui) {
                //ui.helper = $("<div style='background-color: black; width: 40px; height: 40px;'></div>");
                ui.helper.css("background-color", "black");
            
            },*/
            drop: function (event, ui) {
                const sourceNode = $(ui.helper).data("ftSourceNode");
                let dev = { type: sourceNode.key };
                let $dev = createDevice(dev, false, scope);
                const pos = tranformEventCoords(event);
                transform($dev, pos.x, pos.y, transform($dev).rotate);
                addDevice($dev);
                //alert("Added device " + sourceNode.key);
            },
        });
        //-------------------------------------------
        //
