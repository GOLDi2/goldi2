...
var simcir = function ($) {
    ...
    var createDeviceController = function (device) {
        ...
        var createUI = function () {
            ...
            device.$ui.on('deviceAdd', () => {
                ...
                updateLabelPosition = function () {
                    ...
                    $label.attr({ x: anchor.x - $label[0].getBoundingClientRect().width / 2, y: anchor.y + 0.8 * unit });
                };
                updateLabelPosition();
        ...
        var openParameterDialog = function () {
            ...
            const labelDef = { name: "label", type: 'string',
        ...
        var getAnchorBelow = function () {
            ...
            return { x: doff.x + size.width, y: doff.y + size.height };
        };
        var updateTooltipPosition = function () {
            if ($tooltip !== null) {
                ...
                device.$ui.closest(".simcir-workspace").append($tooltip);
            }
        };
    ...
    var createWorkspace = function (data, $container) {
        ...
        var beginPan = function (event) {
            ...
            dragMoveHandler = function (event) {
                transform($containerPane, event.pageX - dragPoint.x, event.pageY - dragPoint.y);
            };
            dragCompleteHandler = function (event) {
                const pos = transform($containerPane);
