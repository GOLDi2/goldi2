...
class TreeController
{
    ...
    private createLibraryNode(name : string, libraryID : string) : any
        {
            return {
                title : name, folder : true, hideCheckbox : true, key : libraryID
            };
        }
    ...
    private getTreeStructure() : Array<any>
        {
            ...
            this.beastController.getLibraries()
                .forEach((lib) =>
                         {
                             const node = this.createLibraryNode(lib.name, lib.ID);
                             treeStructure.push(node);
                             node.children = [];
                             lib.components.forEach((cmp) =>
                                                    {
                                                        node.children.push(this.createComponentNode(cmp.name, new GlobalComponentTypeID(lib.ID, cmp.ID)));
                                                    });
                         });
    ...
    public addLibrary(lib : Library)
        {
            this.tree
                .getRootNode()
                .addChildren(this.createLibraryNode(lib.name, lib.ID));
            this.reloadTree();
        }
...
class DeleteDialog extends Dialog
{
    ...
    protected getContent() : JQuery
        {
            const content : JQuery      = $('<div><p>' + this.message + '</p></div>');
            const deleteButton : JQuery = $('<button>Delete</button>');
            deleteButton.click(() =>
                               {
                                   this.callback();
                                   this.close();
