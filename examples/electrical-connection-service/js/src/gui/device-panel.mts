...
export class EcsDevicePanel extends LitElement {
  private width: number = 1280 - 64 - 8;
  private height: number = 720 - 128 - 8;
  ...
  private updatePosition() {
    ...
    this.positionX = Math.max(Math.min(this.positionX, this.width - 100), 0);
    this.positionY = Math.max(Math.min(this.positionY, this.height - 100), 0);

    if (this.positionX === 0) this.limitXLeft = true;
    else this.limitXLeft = false;
    if (this.positionX === this.width - 100) this.limitXRight = true;
    else this.limitXRight = false;
    if (this.positionY === 0) this.limitYTop = true;
    else this.limitYTop = false;
    if (this.positionY === this.height - 100) this.limitYBottom = true;
